...
const ConfirmSignUpForm: React.FC<{
  ...
  return (
    <div className="auth-form-container">
      ...
      <p className="text-sm mb-2">A confirmation code was sent to <b>{email}</b>.</p>
      ...
      <div className="form-toggle mt-2">
        <button type="button" className="toggle-link" onClick={handleResend} disabled={loading}>
          Resend Code
        </button>
...
const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose }) => {
  ...
  const {
    signIn, signUp, error, loading, clearError,
    needsConfirmation, signUpEmail, confirmSignUp, resendConfirmation
  } = useAuth();
  ...
  const handleSubmit = async (email: string, password: string) => {
    try {
      if (formType === 'signin') {
        ...
        setToast({ message: 'Sign up successful! Please check your email for the confirmation code.', type: 'success' });
  ...
  const handleConfirm = async (code: string) => {
    try {
      ...
      setToast({ message: 'Account confirmed! You can now sign in.', type: 'success' });
      setTimeout(() => {
        setToast(null);
        handleClose();
      }, 2500);
